...
I first learned to use R as a graduate student at Northern Arizona University from [Dr. Philip Turk](http://www.stat.colostate.edu/~pturk/) in 2007. At the time, I never thought that R could have increased in both the number of users and the diversity of people using it; R has been on the rise as a language since 2011. I never would have thought that students taking an introductory statistics course would be encouraged to learn to use R.
...
I believe one of the biggest developments has been the creation of packages to make R easier to work with for newbies. Packages are add-ons created by users of R to increase the functionality of the base R installation. Packages created by Hadley Wickham and others recently have greatly expanded the capabilities of R, while also working to make beginning with R simpler. As of June 2018, [more than 12,600 packages](https://cran.r-project.org/web/packages/index.html) were available on common R repositories.^[You'll see how to download these packages via `install.packages("dplyr")` and load them into your current R working environment via `library("dplyr")`, for example, in Chapter \@ref(rmdanal).]
...
This book was written 3 years ago now and I’d like to do a full rewrite at some point, but that won’t happen this year probably. The server is valuable since installation is really hard for beginners, even with something pretty foolproof like R and RStudio where you just push the buttons for the most part.
