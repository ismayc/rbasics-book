...
This book was last updated by runner on Thursday, August 27, 2026 18:20:58 UTC.
...
It is worth noting that you can't just install RStudio Desktop without first installing R, as RStudio needs to have R installed to run. The steps below walk through both installations, plus two optional additions: a LaTeX distribution if you want to produce PDF documents, and the R packages used later in this book. These steps are adapted from the step-by-step guides that Reed College's Data @ Reed group published at \texttt{reed.edu/data-at-reed/software/R/}, which are no longer online.

\vspace*{0.2in}

\noindent\textbf{Step 1: Install R}\vspace*{0.1in}

Start at \href{https://cran.r-project.org}{CRAN}, the Comprehensive R Archive Network, and download the installer for your operating system.

On a Mac, open \href{https://cran.r-project.org/bin/macosx/}{R for macOS} and choose the build that matches your hardware. Macs with Apple silicon (M1 and later) need the \texttt{arm64} package, while older Intel Macs need the \texttt{x86\_64} package. If you aren't sure which you have, select About This Mac from the Apple menu and read the chip or processor line. Double click the downloaded PKG file and follow the installer.

On a PC, open \href{https://cran.r-project.org/bin/windows/base/}{R for Windows} and click the ``Download R for Windows'' link near the top of the page. Double click the downloaded EXE file and follow the installer.

You do not need to open R after the installation finishes. RStudio will start R for you.

\vspace*{0.2in}

\noindent\textbf{Step 2: Install RStudio Desktop}\vspace*{0.1in}

Go to the \href{https://posit.co/downloads/}{Posit downloads page} and choose the free RStudio Desktop installer listed for your operating system. RStudio is now developed by a company called Posit, which was known as RStudio, Inc.~until 2022, so you will run across both names online (including in a few links later in this book).
...
\noindent\textbf{Step 3: Install LaTeX, but only if you want PDF output}\vspace*{0.1in}
...
The download takes a few minutes and needs no further configuration. If you don't plan to create PDFs, skip this step entirely and select \textbf{HTML} as the Default Output Format for R Markdown. You'll see more about output formats in Chapter \ref{rmarkdown}.

\vspace*{0.2in}

\noindent\textbf{Step 4: Install a few useful packages}\vspace*{0.1in}

The examples in this book use a handful of add-on packages. Run this line in the Console to install them:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{install.packages}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"dplyr"}\NormalTok{, }\StringTok{"ggplot2"}\NormalTok{, }\StringTok{"tidyr"}\NormalTok{, }\StringTok{"knitr"}\NormalTok{, }\StringTok{"rmarkdown"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

You'll see a stream of text in the Console as each package downloads and installs, which is expected and does not signal a problem unless it ends in an error. You only need to install a package once on a given machine, but you'll load it in each new R session with \texttt{library}, as described in Chapter \ref{rmdanal}.

\vspace*{0.2in}

\noindent\textbf{Step 5: Test the installation}\vspace*{0.1in}

To check that the pieces work together, create a small R Markdown document. Select File \textgreater{} New File \textgreater{} R Markdown from the RStudio menu. In the dialog that appears, choose Document on the left, enter a title, and pick the Default Output Format. Choose PDF if you completed Step 3 and want to confirm your LaTeX installation, and otherwise choose HTML.
