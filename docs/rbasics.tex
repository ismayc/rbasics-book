...
some readers. Links to the different YouTube videos directly found in
the HTML version are provided in the PDF version. You can also download
the video files directly
\href{https://github.com/ismayc/rbasics-book/tree/master/mp4}{here}.
Note that no audio is attached to these screencast videos.
...
reproducible research. Many screenshots and screencasts (with no audio)
will be included, but if further clarification is needed on these or any
other aspect of the book, please create a GitHub issue
...
\href{https://code.tutsplus.com/courses/git-basics-github-pull-requests}{Pull
requests} on GitHub for typos or improvements are preferred and you can
easily do so by clicking on the Edit button near Search at the top of
...
Desktop version 1.0 or higher, and \texttt{rmarkdown} R package version
...
This book was last updated by Chester on Sunday, January 07, 2018
17:11:17 PST.
...
referred to as R Markdown) in Chapter \ref{rmarkdown}, and will now
...
login like shown in the screencast below in the Console pane.
...
Below is an example video of sharing this \textbf{initial.Rproj} project
...
place to view the contents of files and objects in R. In the screencast
below, you can see that we can change the text in this file and then
save the file.
...
to check that it works. The example video below shows how this may be
done.
...
what this means in the video below that also includes examples of
invalid names of objects. Note that we will continue to work inside the
R chunk as we did in the last video. You'll see that these code chunks
...
check. Some examples are shown below as a screencast.
...
machine that I do below in the video. That's OK. This is just an example
...
working in RStudio. An example of this error is below in a video.
...
The screencast below shows the process of creating both ordered and
unordered lists.
...
The video below walks through downloading the CSV file and loading it
into the data frame object named \texttt{periodic\_table}. In addition,
it shows another way to view data frames that is built into RStudio
without having to run the \texttt{View} function. Note that a new R
Markdown file is created here as well called
\texttt{chemistry\_example.Rmd}. We are writing the commands directly
into R chunks here, but you may find it nicer to play around in your R
Console sandbox first.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dec_frac_seq <-}\StringTok{ }\KeywordTok{seq}\NormalTok{(}\DataTypeTok{from =} \DecValTok{10}\NormalTok{, }\DataTypeTok{to =} \DecValTok{3}\NormalTok{, }\DataTypeTok{by =} \OperatorTok{-}\FloatTok{0.2}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{friend_names[friend_names }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Bertha"}\NormalTok{, }\StringTok{"Alice"}\NormalTok{)]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{periodic_table[ (periodic_table}\OperatorTok{$}\NormalTok{name }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Hydrogen"}\NormalTok{, }\StringTok{"Oxygen"}\NormalTok{) ), }
...
For references on errors, check out the following two links by Noam Ross
...
Try your best to not be intimidated by R errors. Oftentimes, you will
find that you are able to understand what they mean by carefully reading
over them. When you can't, carefully look over your R Markdown file
again. You might also want to clear out all of your R environment and
start at the top by running the chunks. Remember to only include what
you deem your reader will need to follow your analysis.
